...
export function GraphPage() {
  ...
  const [, setTimelineNodeIds] = useState<string[]>([])
      ...
      >
        <div className="flex items-center justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-4">
            <p className="text-[#8b6355]/80 text-sm hidden sm:block">
              검색 = 기억 그래프를 여는 행위
            </p>
            <Link
              to="/dashboard"
              className="text-sm text-[#8b6355] hover:text-[#e89580] font-medium transition-colors"
            >
              일기 타임라인 →
            </Link>
          </div>
          ...
          {showTimeline && (
            <div className="mt-2 w-64 max-h-64 overflow-y-auto">
              <Timeline
                ...
                onSelectNodeIds={(ids) => setTimelineNodeIds(ids)}
              />
            </div>
          )}
        </div>
